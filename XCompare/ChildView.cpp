...
CMFCRibbonEdit* pRows1;
CMFCRibbonEdit* pCols1;
CMFCRibbonEdit* pRows2;
CMFCRibbonEdit* pCols2;
...
void CChildView::OnPickFirstSheet()
{
	...
	if (tmpWSN > 0)
	{
		...
		tmps.Format(_T("%d"), table1.NumberOfRows);
		pRows1->SetEditText(tmps);
		tmps.Format(_T("%d"), table1.NumberOfColumns);
		pRows1->SetEditText(tmps);
		updateCombos1();
		m_nCellWidth = STEP_X;
...
void CChildView::OnPickSecondSheet()
{
	...
	if (tmpWSN > 0)
	{
		...
		tmps.Format(_T("%d")), table2.NumberOfRows;
		pRows2->SetEditText(tmps);
		tmps.Format(_T("%d")), table2.NumberOfColumns;
		pCols2->SetEditText(tmps);
		m_nCellWidth = STEP_X;
		m_nCellHeight = STEP_Y;
		m_nRibbonWidth = 0;
		m_nViewWidth = STEP_X + OFFSET_X + ((table2.NumberOfColumns + 1) * m_nCellWidth) + m_nRibbonWidth;
...
void CChildView::OnUpdateRows1(CCmdUI *pCmdUI)
{
	if (!(filename1 == "")) pCmdUI->Enable(true); else pCmdUI->Enable(false);
	pRibbon = ((CFrameWndEx*)AfxGetMainWnd())->GetRibbonBar();
	pRows1 = DYNAMIC_DOWNCAST(CMFCRibbonEdit, pRibbon->FindByID(ID_ROWS1));
}
void CChildView::OnRows1()
{
	table1.NumberOfRows = _ttoi(pRows1->GetEditText());
}
void CChildView::OnUpdateCols1(CCmdUI *pCmdUI)
{
	if (!(filename1 == "")) pCmdUI->Enable(true); else pCmdUI->Enable(false);
	pRibbon = ((CFrameWndEx*)AfxGetMainWnd())->GetRibbonBar();
	pCols1 = DYNAMIC_DOWNCAST(CMFCRibbonEdit, pRibbon->FindByID(ID_COLS1));
}
void CChildView::OnCols1()
{
	table1.NumberOfColumns = _ttoi(pCols1->GetEditText());
}
void CChildView::OnUpdateRows2(CCmdUI *pCmdUI)
{
	if (!(filename2 == "")) pCmdUI->Enable(true); else pCmdUI->Enable(false);
	pRibbon = ((CFrameWndEx*)AfxGetMainWnd())->GetRibbonBar();
	pRows2 = DYNAMIC_DOWNCAST(CMFCRibbonEdit, pRibbon->FindByID(ID_ROWS2));
}
void CChildView::OnRows2()
{
	table2.NumberOfRows = _ttoi(pRows2->GetEditText());
}
void CChildView::OnUpdateCols2(CCmdUI *pCmdUI)
{
	if (!(filename2 == "")) pCmdUI->Enable(true); else pCmdUI->Enable(false);
	pRibbon = ((CFrameWndEx*)AfxGetMainWnd())->GetRibbonBar();
	pCols2 = DYNAMIC_DOWNCAST(CMFCRibbonEdit, pRibbon->FindByID(ID_COLS2));
}
void CChildView::OnCols2()
{
	table2.NumberOfColumns = _ttoi(pRows2->GetEditText());
}