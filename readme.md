XCompare is a fast comparison tool for Excel tables.
...
- It provides a fully automatic comparison process if columns' names across tables' are identical. In this case, the tool can mark all discovered differences directly in the Excel files (via OLE) - and, as well as in the 'normal' mode, even cases when columns and/or rows are randomly shuffled.
